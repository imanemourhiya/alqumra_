...
export default function RegisterPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => { register(name, email, password); navigate('/'); }, 900);
  };
  ...
  return (
    <div className="auth-page">
      ...
      <motion.div className="auth-card glass" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <h2 className="auth-title">{t('joinTitle')}</h2>
        <p className="auth-sub">{t('createAccount')}</p>
        {error && <div className="auth-error">{error}</div>}
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="form-group">
            <label className="form-label"><FiUser /> {t('fullName')}</label>
            <input className="form-input" placeholder="Imane Mourhiya" value={name} onChange={e=>setName(e.target.value)} />
          </div>
          <div className="form-group">
          ...
          <div className="register-cols">
            ...
            <div className="form-group">
              <label className="form-label"><FiLock /> {t('confirm')}</label>
              <input type="password" className="form-input" placeholder="••••••••" value={confirm} onChange={e=>setConfirm(e.target.value)} />
            </div>
          </div>
        ...
        <p className="auth-switch">{t('alreadyMember')} <Link to="/login">{t('signInBtn')}</Link></p>
      </motion.div>
    </div>
  );
}